        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {posts.map((post) => (
            <Link key={post.slug} to={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
              <div
                style={{ backgroundColor: 'white', borderRadius: '16px', overflow: 'hidden', border: '1px solid #E5E7EB', display: 'flex' }}
                onMouseEnter={e => (e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,0.08)')}
                onMouseLeave={e => (e.currentTarget.style.boxShadow = 'none')}
              >
                {post.image && (
                  <img src={post.image} alt={post.title} style={{ width: '180px', height: '140px', objectFit: 'cover', flexShrink: 0 }} />
                )}
                <div style={{ padding: '24px', flex: 1 }}>
                  ...
                  <span style={{ color: '#007298', fontSize: '13px', fontWeight: '600' }}>Read more &rarr;</span>
                </div>
              </div>
            </Link>
          ))}
