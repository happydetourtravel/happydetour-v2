...
export default function PanamaCanalLAMiami() {
  const [contactOpen, setContactOpen] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);

  // Confirmed day-by-day from Virgin Voyages' own itinerary page.
  const days = [
    { day: 1, port: "Los Angeles, California, USA", time: "Departs 05:00 PM", desc: "Board the Brilliant Lady in Los Angeles for a true bucket-list voyage through the Panama Canal." },
    { day: 2, port: "At Sea", time: "", desc: "A sea day heading south along the Baja coast." },
    { day: 3, port: "At Sea", time: "", desc: "A second sea day before Cabo." },
    { day: 4, port: "Cabo San Lucas, Mexico", time: "08:00 AM - 05:00 PM", desc: "The iconic El Arco rock formation, Medano Beach, and a lively marina." },
    { day: 5, port: "At Sea", time: "", desc: "A sea day heading toward Guatemala." },
    { day: 6, port: "At Sea", time: "", desc: "A second sea day." },
    { day: 7, port: "At Sea", time: "", desc: "A third sea day before Puerto Quetzal." },
    { day: 8, port: "Puerto Quetzal, Guatemala", time: "08:00 AM - 08:00 PM", desc: "A long day and evening in Guatemala's Pacific gateway port." },
    { day: 9, port: "At Sea", time: "", desc: "A sea day heading toward Costa Rica." },
    { day: 10, port: "Puntarenas, Costa Rica", time: "08:00 AM - 06:00 PM", desc: "Verdant jungles and easy access to Costa Rica's rainforests and wildlife." },
    { day: 11, port: "At Sea", time: "", desc: "A sea day heading toward Panama." },
    { day: 12, port: "Panama City, Panama", time: "09:00 AM - 07:00 PM", desc: "A modern skyline meets colonial history on the edge of the canal itself." },
    { day: 13, port: "Panama Canal (Cruising), Panama", time: "", desc: "A full day cruising the Panama Canal — one of cruising's most iconic bucket-list experiences." },
    { day: 14, port: "Cartagena, Colombia", time: "Arrives 01:00 PM", desc: "The first of two days in Cartagena's walled colonial old town." },
    { day: 15, port: "Cartagena, Colombia", time: "Departs 05:00 PM", desc: "A second day to explore museums, restaurants, and white sand beaches nearby." },
    { day: 16, port: "At Sea", time: "", desc: "A sea day heading toward Miami." },
    { day: 17, port: "At Sea", time: "", desc: "The final sea day before arriving in Miami." },
    { day: 18, port: "Miami, Florida, USA", time: "Arrives 06:30 AM", desc: "Seventeen nights from Los Angeles to Miami, done the Virgin way." },
  ];

  // Verified per-sailor pricing pulled directly from Virgin Voyages / Eric's
  // FirstMates tool. Taxes & fees included. Sea View and Insider are sold out
  // on this sailing.
  const cabins = [
    { type: "Insider", desc: "Roomy rainshower, mood lighting, glam station, large flat screen TV.", price: "Sold Out", soldOut: true },
    { type: "Sea View", desc: "Porthole window for ocean views, roomy rainshower, mood lighting.", price: "Sold Out", soldOut: true },
    { type: "Sea Terrace", desc: "Private balcony with hammock, roomy rainshower, mood lighting.", price: "$5,120" },
    { type: "RockStar Quarters", desc: "Exclusive Richard's Rooftop access, stocked in-room bar, priority booking.", price: "$9,503" },
    { type: "Mega RockStar Quarters", desc: "Largest interior/exterior spaces, daily bar tab & spa access, bottomless in-room bar.", price: "$15,283" },
  ];

  const bookingUrl = "https://www.virginvoyages.com/book/voyage-planner/pre-checkout?agencyId=589&agentId=278796&currencyCode=USD&dateFrom=2026-10-01&dateTo=2026-10-31&durations=13%2B&packageCode=17NLAXH&voyageId=BR26102917NLAXH";

  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#F9FAFB", fontFamily: "sans-serif" }}>
      <div className="h-[320px] md:h-[480px]" style={{ position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, backgroundImage: "url('/brilliantlady.jpeg')", backgroundSize: "cover", backgroundPosition: "center", filter: "brightness(0.55)" }} />
        ...
        <div className="px-4 md:px-6 pb-8 md:pb-12" style={{ position: "relative", maxWidth: "1280px", margin: "0 auto", height: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
          ...
          <h1 className="text-[28px] md:text-[42px]" style={{ color: "white", fontWeight: "800", margin: "0 0 12px 0", lineHeight: "1.2" }}>Panama Canal: LA to Miami</h1>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "20px", alignItems: "center" }}>
            <span style={{ color: "rgba(255,255,255,0.9)", fontSize: "15px" }}>October 29, 2026</span>
            <span style={{ color: "rgba(255,255,255,0.9)", fontSize: "15px" }}>17 Nights</span>
            <span style={{ color: "rgba(255,255,255,0.9)", fontSize: "15px" }}>Brilliant Lady</span>
            <span style={{ color: "rgba(255,255,255,0.9)", fontSize: "15px" }}>Departs Los Angeles</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 md:px-6 py-10 md:py-16" style={{ maxWidth: "1280px", margin: "0 auto" }}>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 lg:gap-12" style={{ alignItems: "start" }}>
          <div>
            ...
            <div style={{ marginBottom: "32px", backgroundColor: "#FEF3C7", border: "1px solid #FCD34D", borderRadius: "10px", padding: "14px 18px" }}>
              <p style={{ color: "#92400E", fontSize: "13px", margin: 0, fontWeight: "600" }}>Sea View and Insider cabins are already sold out on this sailing — Sea Terrace and up only.</p>
            </div>
            ...
            <div style={{ marginBottom: "48px" }}>
              ...
              <h2 style={{ color: "#1F2937", fontSize: "28px", fontWeight: "800", margin: "0 0 16px 0" }}>Seventeen nights from Los Angeles to Miami</h2>
              <p style={{ color: "#4B5563", fontSize: "16px", lineHeight: "1.8", margin: "0 0 16px 0" }}>
                A true bucket-list voyage: Cabo San Lucas, Puerto Quetzal, Puntarenas, Panama City, a full day cruising the Panama Canal itself, and two days in Cartagena before finishing in Miami. Available only twice a year on Brilliant Lady.
              </p>
              <p style={{ color: "#4B5563", fontSize: "16px", lineHeight: "1.8", margin: 0 }}>
                This is the longest sailing on this page, with plenty of sea days to enjoy the ship between an unusually varied lineup of ports. All specialty restaurants, WiFi, and fitness classes are included. Gratuities are paid onboard separately.
              </p>
            </div>
            ...
            <div style={{ backgroundColor: "white", borderRadius: "16px", border: "1px solid #E5E7EB", overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
              <div style={{ backgroundColor: "#1F2937", padding: "16px 20px" }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: "8px" }}>
                  <span style={{ color: "white", fontSize: "36px", fontWeight: "800" }}>$5,120</span>
                  <span style={{ color: "#9CA3AF", fontSize: "13px" }}>/person</span>
                </div>
                <p style={{ color: "#FDE68A", fontSize: "12px", margin: "6px 0 0 0" }}>Per person, double occupancy. Sea Terrace and up only — Sea View and Insider sold out.</p>
